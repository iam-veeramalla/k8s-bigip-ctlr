...
func NewCRManager(params Params) *CRManager {
	...
	crMgr.resourceSelector, _ = createLabelSelector(params.ResourceLabel)
...
func (crMgr *CRManager) watchingAllNamespacesLocked() bool {
...
func (crMgr *CRManager) addNamespacedInformer(
	...
	if crMgr.watchingAllNamespacesLocked() {
		return fmt.Errorf(
			"Cannot add additional namespaces when already watching all.")
	}
	if len(crMgr.crInformers) > 0 && "" == crMgr.namespaces[0] {
		return fmt.Errorf(
...
func (crMgr *CRManager) newInformer(
	...
	log.Debugf("Creating new CR informer")
...
func (crInfr *CRInformer) start() {
	if crInfr.vsInformer != nil {
		crInfr.vsInformer.Run(crInfr.stopCh)
	}
}
